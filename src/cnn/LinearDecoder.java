...
public class LinearDecoder implements DiffFunction{
    ...
    public DoubleMatrix[][] getThetaArr() {
        int channels = theta1.rows/(patchSize*patchSize);
        DoubleMatrix[][] res = new DoubleMatrix[theta1.columns][channels];
        for(int i = 0; i < theta1.columns; i++) {
            for(int j = 0; j < channels; j++) {
                res[i][j] = theta1.getRange(patchSize*patchSize*j,patchSize*patchSize*(j+1),i,i+1).reshape(patchSize, patchSize);
            }
        }
        return res;
    }

    public double[] getBiasArr() {
        return bias1.data;
    }
	...
	protected CostResult[] cost(DoubleMatrix input, DoubleMatrix theta1, DoubleMatrix theta2, DoubleMatrix bias1, DoubleMatrix bias2) {
        ...
        delta2.muli(Utils.activationFunction(layer1, result[0][1], a));
		//W2grad
	...
	public DoubleMatrix backpropagation(DoubleMatrix input, DoubleMatrix delta3, double momentum, double alpha) {
        ...
        delta2.muli(Utils.activationFunction(layer1, input, a));
        ...
        //agrad
        //double aGrad = Utils.aGrad(layer1, )
