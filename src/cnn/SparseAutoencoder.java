...
public class SparseAutoencoder implements DiffFunction{
    ...
    private int layer2 = Utils.PRELU;
    private double a;
	...
	private void initializeParams() {
        ...
        double stdev = 0.01;//Math.sqrt(2.0/(hiddenSize));
		theta1 = DoubleMatrix.randn(inputSize, hiddenSize).muli(stdev);
	...
	public void gradientCheck(DoubleMatrix[][] input, DoubleMatrix labels, CostResult cr, NeuralNetwork cnn) {
        ...
        a += epsilon;
        CostResult costP = cnn.computeCost(input, labels);
        a -= 2*epsilon;
        CostResult costM = cnn.computeCost(input, labels);
        a += epsilon;
        double aG = (costP.cost-costM.cost)/(2*epsilon);
        System.out.println("SAE a: "+Math.abs((cr.aGrad-aG)/(cr.aGrad+aG)));
	}
	...
	protected CostResult[] cost(DoubleMatrix input, DoubleMatrix output, DoubleMatrix theta1, DoubleMatrix theta2, DoubleMatrix bias1, DoubleMatrix bias2) {
		...
		delta3.muli(Utils.activationGradient(layer1, result[0][2], a));
		//sparsity term
		...
		delta2.muli(Utils.activationGradient(layer2, result[0][1], a));
		//W2grad
	...
	public CostResult cost(DoubleMatrix input, DoubleMatrix output, DoubleMatrix delta3) {
        ...
        //output.divi(a*2);
        //delta3.divi(a*2);
		delta3.muli(Utils.activationGradient(layer1, output, a));
        ...
        double aGrad = Utils.aGrad(layer1, output, delta3, a);

		return new CostResult(0, thetaGrad, biasGrad, delta2, aGrad);
	}
	...
	private DoubleMatrix[][] feedForward(DoubleMatrix patches, DoubleMatrix theta1, DoubleMatrix theta2, DoubleMatrix bias1, DoubleMatrix bias2) {
		...
		//a2
		result[0][1] = Utils.activationFunction(layer1, result[1][1], a);
		//z3
		...
		//a3
		result[0][2] = Utils.activationFunction(layer2, result[1][2], a);
		return result;
	}
	...
	public DoubleMatrix compute(DoubleMatrix input) {
		...
		return Utils.activationFunction(layer1, result, a);
	}
